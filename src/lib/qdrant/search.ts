...
/**
 * Search specifically for Transaction Theory related documents
 * @param collectionName - Name of the collection to search in
 * @param query - Search query text
 * @param limit - Maximum number of results to return
 * @param embeddingModel - Embedding model to use (optional)
 * @returns Array of search results
 */
export const searchTransactionTheory = async (
  collectionName: string,
  query: string,
  limit: number = 3,
  embeddingModel?: string
  ...
  try {
    // Create a filter for Transaction Theory documents
    const filter = {
      should: [
        {
          key: 'metadata.tags',
    ...
    // Call the existing filtered search function
    return await searchDocumentsWithFilter(
      collectionName,
      query,
      filter,
      limit,
    ...
    console.error('Error searching Transaction Theory documents:', error);
    throw error;
  }
}; 